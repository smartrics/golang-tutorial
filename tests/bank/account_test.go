...
func TestNewAccountRequiresIDAndAmount(t *testing.T) {
	acc := bank.NewBankAccount("ABC-1", 1000.0)
	if acc.ID() != "ABC-1" {
...
func TestDepositNegativeAmountFails(t *testing.T) {
	...
	if !errors.Is(err, bank.ErrDepositNegativeAmount) {
...
func TestWithdrawNegativeAmountFails(t *testing.T) {
	...
	if !errors.Is(err, bank.ErrWithdrawNegativeAmount) {
...
func TestNewCheckingAccount(t *testing.T) {
	ca, err := bank.NewCheckingAccount("ABC-1", 1000.0, 100.0)
	if err != nil {
		t.Errorf("Expected no error, got '%v'", err)
	}
...
func TestNewCheckingAccountNegativeAmountsWithinOverdraftSucceeds(t *testing.T) {
	ca, err := bank.NewCheckingAccount("ABC-1", -50.0, 100.0)
	if err != nil {
		t.Errorf("Expected no error, got '%v'", err)
	}
	if ca.Balance() != -50.0 {
...
func TestNewCheckingAccountWithBalanceLowerThanOverdraftFails(t *testing.T) {
	ca, err := bank.NewCheckingAccount("ABC-1", -200.0, 100.0)
	if err == nil {
...
func TestNewCheckingAccountWithdrawWithinOverdraftSucceeds(t *testing.T) {
	ca, _ := bank.NewCheckingAccount("ABC-1", 1000.0, 100.0)
...
func TestSavingAccountApplyInterest(t *testing.T) {
	sa, _ := bank.NewSavingAccount("ABC-1", 1000.0, 0.05)
	sa = sa.ApplyInterest()
